...
{
    [Fact]
    public void Application_project_declares_release_version_0_3_4()
    {
        var properties = LoadApplicationProjectProperties();

        Assert.Equal("0.3.4", properties["Version"]);
    }
    ...
    public void Installer_default_version_matches_the_application_version()
    {
        ...
        Assert.Contains("#define AppVersion \"0.3.4\"", installer, StringComparison.Ordinal);
    }

    [Fact]
    public void Local_release_requires_and_publishes_the_matching_changelog_section()
    {
        var codexHpRoot = FindCodexHpRoot();
        var changelogPath = Path.Combine(codexHpRoot, "CHANGELOG.md");
        ...
        Assert.Contains("## [0.3.4] - 2026-09-02", changelog, StringComparison.Ordinal);
